...
from TemplateModels.BinaryClassification import BinaryClassification
...
# load datasets - set (VALIDATION & TEST) datasets and dataloaders to
# 'None' to disable either or both validation and testing
...
# runs after every mini-batches (compares training reports)
VALIDATION_DATASET = torchvision.datasets.ImageFolder(
    os.path.join(
        "data", "microsoft-catsvsdogs-dataset", "PetImages",
        "partitioned-dataset", "validation_dataset"
    ),
    transform=transforms.Compose([
        transforms.Resize((INPUT_DIM, INPUT_DIM)),
        transforms.ToTensor()
...
# runs after every epochs
TEST_DATASET = torchvision.datasets.ImageFolder(
    os.path.join(
        "data", "microsoft-catsvsdogs-dataset", "PetImages",
        "partitioned-dataset", "testing_dataset"
    ),
    transform=transforms.Compose([
        transforms.Resize((INPUT_DIM, INPUT_DIM)),
        transforms.ToTensor()
...
def test_loop(dataset, dataloader):
    ...
    with torch.no_grad():
        for batch_inputs, batch_labels in dataloader:
            ...
            correct += ((batch_output > 0.5) == (batch_labels > 0.5)).type(torch.float).sum().item()
...
if __name__ == "__main__":
    ...
    writer.add_graph(MODEL, torch.rand(1, 3, INPUT_DIM, INPUT_DIM))
    ...
    while epoch_idx < TRAINING_EPOCHS:
        ...
        for batch_idx, batch_data in enumerate(TRAIN_DATALOADER):
            ...
            if batch_idx % MINI_BATCH_LOG_INTERVAL == MINI_BATCH_LOG_INTERVAL - 1:
                ...
                # if there is a Validation Dataset perform validation runs and write reports to Tensorboard
